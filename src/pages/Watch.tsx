...
import { ArrowLeft, Heart, Flag, Crown, Tv, Send, CheckCircle, Radio, Play, ChevronLeft, AlertCircle, Loader2, ArrowRight, Share2, Lock } from 'lucide-react';
...
export default function Watch() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(async ([ch, all]) => {
        setChannel(ch);
        setAllChannels(all.filter((c: Channel) => c.id !== id));
  ...
  const youMightLike = allChannels
    .slice(0, 6);
  const latestAdded = [...allChannels]
    .slice(0, 6);
    ...
    <div className="min-h-screen pt-20 sm:pt-28 pb-10 sm:pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="relative w-full max-w-md card p-8 shadow-2xl border-amber-500/20">
              ...
              <form onSubmit={submitReport}>
                ...
                <textarea
                  ...
                  placeholder="Décrivez le problème (coupure, son décalé, mauvaise chaîne...)"
                  required
                  autoFocus
                />
                {reportError && (
              ...
              <div className="flex items-center gap-4 sm:gap-5">
                {channel.logo_url ? (
                  <div className="relative w-16 h-16 shrink-0 bg-white/5 rounded-xl p-2 border border-white/10 shadow-lg">
                    <img loading="lazy" src={channel.logo_url} alt={channel.name} className="w-full h-full object-contain drop-shadow-lg" />
                  </div>
                ) : (
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    <span className="inline-flex px-2.5 py-0.5 rounded-lg bg-white/5 border border-navy-800 text-xs font-semibold text-navy-200">
                      {channel.category || 'TV'}
                    </span>
        ...
        <div className="mt-12 space-y-12">
          ...
          <ChannelSection title="Derniers ajouts" channels={latestAdded} />
        ...
        {channels.map((ch) => (
          <Link key={ch.id} to={`/watch/${ch.id}`} className="card-hover p-3 flex flex-col items-center gap-3 group">
            <div className="w-16 h-16 bg-navy-950 rounded-2xl flex items-center justify-center border border-navy-800 group-hover:border-brand-500/30 transition-colors p-2 shrink-0 relative overflow-hidden shadow-lg">
              {ch.logo_url ? <img loading="lazy" src={ch.logo_url} alt="" className="w-full h-full object-contain" /> : <Tv className="w-8 h-8 text-brand-500/50 group-hover:text-brand-400" />}
            </div>
