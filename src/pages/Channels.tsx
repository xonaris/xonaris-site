...
import { Search, Tv, Play, Crown, Radio, X, MonitorPlay, AlertCircle, RefreshCw } from 'lucide-react';
import { useDocumentTitle } from '../hooks/useDocumentTitle';

const categories = ['Toutes', 'Généraliste', 'Sport', 'Info', 'Divertissement', 'Cinéma', 'Musique', 'Jeunesse', 'Documentaire'];
...
export default function Channels() {
  ...
  const filtered = channels.filter((ch) => {
    ...
    const matchCat = category === 'Toutes' || ch.category === category;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col gap-6 sm:gap-8 mb-6 sm:mb-10 animate-fade-down">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 sm:gap-6">
            <div>
              <div className="inline-flex items-center gap-2 px-2.5 sm:px-3 py-1 rounded-full bg-brand-500/10 border border-brand-500/20 text-brand-400 text-[10px] sm:text-xs font-bold uppercase tracking-wider mb-3 sm:mb-4">
              ...
              <p className="text-navy-300 text-sm sm:text-lg font-medium max-w-xl">
                Accédez à {channels.length > 0 ? `plus de ${channels.length}` : 'vos'} chaînes en direct avec une qualité optimale et sans coupure.
              </p>
            </div>
          ...
          {/* Categories */}
          <div className="flex flex-nowrap items-center gap-2 sm:gap-3 overflow-x-auto pb-2 sm:pb-4 scrollbar-none snap-x w-full">
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`snap-start flex flex-shrink-0 items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-5 py-2 sm:py-2.5 rounded-lg sm:rounded-xl text-xs sm:text-base font-semibold whitespace-nowrap transition-all duration-300 ${
                    isActive
                ...
                >
                  ...
                  {catCounts[cat] > 0 && (
                    <span className={`text-[11px] px-2 py-0.5 rounded-full ${
                      isActive ? 'bg-navy-950/20 text-[#050505]' : 'bg-white/10 text-navy-200'
                    }`}>
                      {catCounts[cat]}
                    </span>
                  )}
        ...
        {(search || category !== 'Toutes') && (
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-6 sm:mb-8 animate-fade-up">
            <span className="text-xs sm:text-sm font-medium text-navy-400 flex items-center gap-1.5">
