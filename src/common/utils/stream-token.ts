...
export function generateStreamToken(userId: string, channelId: string, ttlSeconds = 60): string {
  ...
  if (secret === 'CHANGE_ME_STREAM_SECRET_KEY') {
    if (process.env.NODE_ENV === 'production') {
      throw new Error('STREAM_SIGN_SECRET must be changed from default value in production');
    }
    console.warn('⚠️  STREAM_SIGN_SECRET is using default value. Change it for production.');
  }
