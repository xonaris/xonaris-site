...
export function initRsaKeyPair(): void {
  ...
  if (envPublic && envPrivate) {
    keyPair = {
      publicKey: normalizePem(envPublic),
      privateKey: normalizePem(envPrivate),
    };
    return;
  }
  ...
  // Note: ephemeral RSA key pair generated — set RSA_PUBLIC_KEY / RSA_PRIVATE_KEY env vars for production
}
