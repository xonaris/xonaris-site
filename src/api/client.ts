...
async function tryRefreshTokens(): Promise<boolean> {
  ...
  refreshPromise = api
    .post('/auth/refresh', {}, { _skipAuthRefresh: true } as any)
    .then(() => {
      refreshPromise = null;
      return true;
    })
...
async function encryptWithFreshKey<T>(
  nonce: string,
  operation: () => Promise<T>,
): Promise<T> {
  try {
    ...
    // A stale or failed prefetch should not permanently block the first real request.
...
api.interceptors.request.use(async (config: InternalAxiosRequestConfig) => {
  ...
  if (hasBody) {
    ...
    try {
      const payload = await encryptWithFreshKey(nonce, () =>
        encryptRequestBody(JSON.stringify(config.data), nonce),
      );
      config.data = payload;
    } catch (err) {
      cleanupPendingKey(nonce);
    ...
    try {
      const encKey = await encryptWithFreshKey(nonce, () =>
        encryptKeyForHeaders(nonce),
      );
