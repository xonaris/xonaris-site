...
export class AdminController {
  ...
  async getStatsHistory(@Query('days') days?: string) {
    ...
    return this.adminService.getStatsHistory(d);
  }
  ...
  async getUsers(
    ...
    return this.adminService.getUsers(
      ...
      limit ? Math.min(100, Math.max(1, parseInt(limit, 10) || 50)) : 50,
      search || undefined,
    );
  }
  ...
  @Delete('users/:id')
  async deleteUser(@Param('id', ParseUuidPipe) id: string, @CurrentUser('id') currentUserId: string) {
    return this.adminService.deleteUser(id, currentUserId);
  }

  @Patch('users/:id/ban')
  async banUser(@Param('id', ParseUuidPipe) id: string, @Body() dto: BanUserDto, @CurrentUser('id') currentUserId: string) {
    return this.adminService.banUser(id, dto, currentUserId);
  }
  ...
  async getChannels(
    ...
    @Query('status') status?: 'ALL' | 'ACTIVE' | 'INACTIVE' | 'PREMIUM',
    @Query('sortKey') sortKey?: 'name' | 'sort_order' | 'category' | 'created_at',
  ...
  @Put('channels/:id')
  async updateChannel(@Param('id', ParseUuidPipe) id: string, @Body() dto: UpdateChannelDto) {
  ...
  async getChannelSources(@Param('id', ParseUuidPipe) channelId: string) {
    return this.adminService.getChannelSources(channelId);
  }

  @Post('channels/:id/sources')
  async createChannelSource(@Param('id', ParseUuidPipe) channelId: string, @Body() dto: CreateChannelSourceDto) {
  ...
  async deletePremiumCode(@Param('id', ParseUuidPipe) id: string) {
  ...
  @Patch('reports/:id')
  async updateReport(@Param('id', ParseUuidPipe) id: string, @Body() dto: UpdateReportDto) {
  ...
  @Put('ads/:id')
  async updateAd(@Param('id', ParseUuidPipe) id: string, @Body() dto: UpdateAdDto) {
